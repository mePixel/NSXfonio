...
webhooksRouter.post("/fonio", async (req, res, next) => {
  try {
    ...
    const callStatus =
      payload?.status ?? payload?.callStatus ?? payload?.outcome;
    ...
    if (offerId && callStatus) {
      ...
      if (offer && offer.status === "calling") {
        const noAnswer = [
          ...
          "missed",
          "unanswered",
        ].includes(String(callStatus).toLowerCase());
        ...
        if (noAnswer) {
          ...
          try {
            ...
            console.error(
              "[webhook] Failed to send email due to context fetch error:",
              emailContextError.message,
            );
          }
